...
def mostrar_datos_solicitud_ejecutivo(*,solicitud: pd.Series, is_main: bool = False) -> None:
    ...
    with st.expander(expander_name, expanded=is_main, key=expander_key, on_change="rerun"):
        ...
        if st.session_state.get(expander_key, False) and not (solicitud_ya_gestionada):
            ...
            if ("Estado_Comite" in solicitud['Metadata_Solicitud']) or ("Estado_Titular_Ilocalizable" in solicitud['Metadata_Solicitud']):
                motivo_estado = "Bajo Comité" if ("Estado_Comite" in solicitud['Metadata_Solicitud']) else "Titular Ilocalizable"
                estado_key = "Estado_Comite" if ("Estado_Comite" in solicitud['Metadata_Solicitud']) else "Estado_Titular_Ilocalizable"
                estado_actual = solicitud['Metadata_Solicitud'][estado_key]
        
                # Caso 1: Es 1 (Significa que no la ha visto el negociador)
                if estado_actual == 1:
                    st.warning("El Negociador no ha aprobado/desaprobado el estado {}".format(
                        motivo_estado
                        ),
                    icon="🤔",
                    )
                    ...
                    st.error(
                        "El Negociador rechazó la Petición ({})".format(motivo_estado),
                        icon="❌",
                    )
                elif estado_actual == 3:
                    st.success(
                        "El Negociador aprobó la Petición ({})".format(motivo_estado),
                        icon="✅",
                    )
        ...
        if es_solicitud_sin_responder(solicitud=solicitud):
            if solicitud['Metadata_Solicitud'].get('Comentario_Ejecutivo'):
                st.warning("Comentario Ejecutivo: {}".format(
                    solicitud['Metadata_Solicitud']['Comentario_Ejecutivo']
                ),icon="🔴")
            return
        st.divider()
